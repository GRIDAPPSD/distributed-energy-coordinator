...
def index_info(branch: dict, bus: dict) -> Tuple[dict, dict]:
    for i, name in enumerate(bus, 1):
        bus[name]["idx"] = i

    for i, name in enumerate(branch, 1):
        branch[name]["idx"] = i
    ...
    for line_id in line_ids:
        ...
        for line_phs in line.ACLineSegmentPhases:
            if not branch_info[line.name]["phases"]:
                ...
                branch_info[line.name]["phases"].append(Phase[line_phs.phase[0]])
        ...
        for load_phs in load.EnergyConsumerPhase:
            if load_phs.phase:
                ...
                if load_phs.phase[0] == "A":
                    ...
                    bus_info[node.name]["phases"].append(Phase[load_phs.phase[0]])
