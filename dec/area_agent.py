...
class AreaAgent(object):
    ...
    def alpha_area(self, branch_sw_data_case, bus_info, agent_bus, agent_bus_idx, vsrc, pf_flag, dist_flag, area_number = 0, \
                         alpha_neigh=0, lamda_volt = 0, lamda_alpha = 0, lamdaPi=0, lamdaQi = 0, mu_v_alpha=[0,0,0,0], child =[], lastPi = {}, lastQi = {}, lastVi=[]):
        ...
        n = (nbus_ABC * 3 + nbus_s1s2) + (nbus_ABC * 6 + nbus_s1s2 * 2) + (nbranch_ABC * 6 + nbranch_s1s1 * 2) + \
            (nbus_ABC + nbus_s1s2) + nbus_ABC * 3 + nbus_s1s2  ## (introduce new variables for reactive power injections from inverters)
        #         alpha_PV                  Q_inv
        ...
        if dist_flag:
            ...
            obj_term_2 =  lamda_volt[0]*(x[agent_bus_idx]              - vsrc[0]**2) \
            ...
            for child_bus in child:
                ...
                obj_term_2 += lamdaPi[child_bus][0]*(x[nbus_ABC * 3 + nbus_s1s2 + nbus_ABC * 0 + val_bus['idx']] - val_bus['pq'][0][0]*baseS) \
                            + lamdaPi[child_bus][1]*(x[nbus_ABC * 3 + nbus_s1s2 + nbus_ABC * 1 + val_bus['idx']] - val_bus['pq'][1][0]*baseS) \
                            + lamdaPi[child_bus][2]*(x[nbus_ABC * 3 + nbus_s1s2 + nbus_ABC * 2 + val_bus['idx']] - val_bus['pq'][2][0]*baseS)
                                      
            for child_bus in child:
                val_bus = bus_info[child_bus] 
                obj_term_2 += lamdaQi[child_bus][0]*(x[nbus_ABC * 3 + nbus_s1s2 + nbus_ABC * 3 + val_bus['idx']] - val_bus['pq'][0][1]*baseS) \
                            + lamdaQi[child_bus][1]*(x[nbus_ABC * 3 + nbus_s1s2 + nbus_ABC * 4 + val_bus['idx']] - val_bus['pq'][1][1]*baseS) \
                            + lamdaQi[child_bus][2]*(x[nbus_ABC * 3 + nbus_s1s2 + nbus_ABC * 5 + val_bus['idx']] - val_bus['pq'][2][1]*baseS)
            ...
            obj_term_3 = 0 
            obj_term_3  = 0.5 * mu1_v * 1 * ((x[agent_bus_idx]              - vsrc[0]**2) ** 2) \
                        + 0.5 * mu1_v * 1 * ((x[agent_bus_idx + nbus_ABC]   - vsrc[1]**2) ** 2) \
                        + 0.5 * mu1_v * 1 * ((x[agent_bus_idx + nbus_ABC*2] - vsrc[2]**2) ** 2)
            
            for child_bus in child:
                val_bus = bus_info[child_bus]  
                obj_term_3  += 0.5 * mu1_Pi * ((x[nbus_ABC * 3 + nbus_s1s2 + nbus_ABC * 0 + val_bus['idx']] - val_bus['pq'][0][0]*baseS)**2) \
                            +  0.5 * mu1_Pi * ((x[nbus_ABC * 3 + nbus_s1s2 + nbus_ABC * 1 + val_bus['idx']] - val_bus['pq'][1][0]*baseS)**2) \
                            +  0.5 * mu1_Pi * ((x[nbus_ABC * 3 + nbus_s1s2 + nbus_ABC * 2 + val_bus['idx']] - val_bus['pq'][2][0]*baseS)**2)    
                            
            for child_bus in child:
                val_bus = bus_info[child_bus]  
                obj_term_3  += 0.5 * mu1_Qi * ((x[nbus_ABC * 3 + nbus_s1s2 + nbus_ABC * 3 + val_bus['idx']] - val_bus['pq'][0][1]*baseS)**2) \
                            +  0.5 * mu1_Qi * ((x[nbus_ABC * 3 + nbus_s1s2 + nbus_ABC * 4 + val_bus['idx']] - val_bus['pq'][1][1]*baseS)**2) \
                            +  0.5 * mu1_Qi * ((x[nbus_ABC * 3 + nbus_s1s2 + nbus_ABC * 5 + val_bus['idx']] - val_bus['pq'][2][1]*baseS)**2)     
                            
            for bus in range(5):
                if bus != area_number-1:
                    obj_term_3 += 0.5 * mu1_alpha * ((x[n_alpha] - alpha_neigh[bus]) ** 2)
                # else:
                #     obj_term_3 += 1 * mu1_alpha * ((x[n_alpha] - alpha_neigh[bus]) ** 2)
            
            obj_term_4 = 0 
            obj_term_4  = 0.5 * mu1_v * 1 * ((x[agent_bus_idx]              - lastVi[0]**2)**2) \
                        + 0.5 * mu1_v * 1 * ((x[agent_bus_idx + nbus_ABC]   - lastVi[1]**2)**2) \
                        + 0.5 * mu1_v * 1 * ((x[agent_bus_idx + nbus_ABC*2] - lastVi[2]**2)**2)    
                        
            obj_term_4  += 0.5 * mu1_alpha * ((x[n_alpha] - alpha_neigh[area_number-1])**2)
        ...
        for k, val_br in branch_sw_data_case.items():
            ...
            if val_br['type'] not in secondary_model:
                ...
                A, b = voltage_cons_pri(A, b, idx, nbus_ABC + val_br['from'], nbus_ABC + val_br['to'], counteq, pba, qba,
                                    pbb, qbb, pbc, qbc)
                counteq += 1
                # Phase C
        ...
        for k, val_br in branch_sw_data_case.items():
            ...
            if val_br['type'] in secondary_model:
                if val_br['type'] == 'SPLIT_PHASE':
                    ...
                    A, b = voltage_cons(A, b, idx - nbranch_ABC, from_bus, to_bus, counteq, p_pri, q_pri, p_sec, q_sec) ## Monish
                    counteq += 1
                ...
                if val_br['type'] != 'SPLIT_PHASE':
                    ...
                    A, b = voltage_cons(A, b, idx - nbranch_ABC, from_bus, to_bus, counteq, p_s1, q_s1, p_s2, q_s2) ## Monish
                    counteq += 1
            idx += 1
        ...
        for k in range(n_flow_ABC, n_flow_ABC + nbranch_ABC):
            ...
            line_flow[name[i]]['C'] = [x.value[k + nbranch_ABC * 2] * mul * 1000, x.value[k + nbranch_ABC * 5] * mul * 1000]
            i += 1
        ...
        for child_bus in child:
            val_bus = bus_info[child_bus] 
            lastPi[child_bus][0] = x.value[nbus_ABC * 3 + nbus_s1s2 + nbus_ABC * 0 + val_bus['idx']] * mul * 1000
            lastPi[child_bus][1] = x.value[nbus_ABC * 3 + nbus_s1s2 + nbus_ABC * 1 + val_bus['idx']] * mul * 1000
            lastPi[child_bus][2] = x.value[nbus_ABC * 3 + nbus_s1s2 + nbus_ABC * 2 + val_bus['idx']] * mul * 1000
            
        for child_bus in child:
            val_bus = bus_info[child_bus] 
            lastQi[child_bus][0] = x.value[nbus_ABC * 3 + nbus_s1s2 + nbus_ABC * 3 + val_bus['idx']] * mul * 1000
            lastQi[child_bus][1] = x.value[nbus_ABC * 3 + nbus_s1s2 + nbus_ABC * 4 + val_bus['idx']] * mul * 1000
            lastQi[child_bus][2] = x.value[nbus_ABC * 3 + nbus_s1s2 + nbus_ABC * 5 + val_bus['idx']] * mul * 1000
